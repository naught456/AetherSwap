...
def send_pushplus(token: str, title: str, content: str, template: str = "html") -> bool:
    ...
    try:
        ...
        return False
def infer_smtp_host(imap_server: str, email_user: str) -> str:
    """推导 SMTP 服务器：优先复用 imap 服务器地址（imap.xx → smtp.xx），
    否则按邮箱域名猜测。"""
    server = (imap_server or "").strip()
    if server.startswith("imap."):
        return "smtp." + server[len("imap."):]
    parts = (email_user or "").split("@", 1)
    if len(parts) == 2 and parts[1].strip():
        return "smtp." + parts[1].strip()
    return "smtp.qq.com"
def send_smtp_mail(
    host: str,
    user: str,
    ...
    """通过 SMTP 发送一封纯文本邮件。依次尝试 SSL(465) 与 STARTTLS(587)。"""
        ...
        try:
            ...
            if server is not None:
                try:
                    ...
                    pass
    return False
def send_payment_confirmed_email(config: dict, timeout: float = 15.0) -> bool:
    """点击「已完成支付」后，用系统设置里配置的邮箱凭据（QQ 邮箱授权码可同时
    用于 IMAP 与 SMTP）向该邮箱自己发送一封主题与正文均为「已确认成功付款」的邮件。"""
    notify_cfg = (config or {}).get("notify") or {}
    user = (notify_cfg.get("email_user") or "").strip()
    password = (notify_cfg.get("email_pass") or "").strip()
    if not user or not password:
        return False
    subject = (notify_cfg.get("subject_success") or "已确认成功付款").strip()
    return send_smtp_mail(
        host=infer_smtp_host(notify_cfg.get("imap_server") or "", user),
        user=user,
        password=password,
        to_addrs=[user],
        subject=subject,
