...
async function sendPaymentConfirmedMail() {
  const btn = el("btn-mail-paid");
  const originalText = btn?.textContent || "发送已付款邮件";
  if (btn) {
    btn.disabled = true;
    btn.textContent = "发送中…";
  }
  try {
    const data = await fetchJson(API + "/send_payment_confirmed_mail", { method: "POST" });
    if (data && data.ok === true) {
      toast("已发送", "「已确认成功付款」邮件已发送到配置邮箱");
    } else {
      toast("发送失败", "请检查系统设置中的邮箱账号/授权码，并确认已开启 SMTP 服务");
    }
  } catch (e) {
    toast("发送异常", e.message || "请检查后端服务是否在运行");
  } finally {
    if (btn) {
      btn.disabled = false;
...
function bindEvents() {
  ...
  el("btn-mail-paid")?.addEventListener("click", sendPaymentConfirmedMail);
